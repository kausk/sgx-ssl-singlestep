...
sgx_status_t sign_single(sgx_enclave_id_t eid, BIGNUM* retval, unsigned char* message, int len)
{
	sgx_status_t status;
	ms_sign_single_t ms;
...
sgx_status_t get_ECDSA_sign_ADDR(sgx_enclave_id_t eid, void** retval)
{
	sgx_status_t status;
	ms_get_ECDSA_sign_ADDR_t ms;
	status = sgx_ecall(eid, 4, &ocall_table_TestEnclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t get_BN_mod_aq_ADDR(sgx_enclave_id_t eid, void** retval)
{
	sgx_status_t status;
	ms_get_BN_mod_aq_ADDR_t ms;
	status = sgx_ecall(eid, 5, &ocall_table_TestEnclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t get_BN_usub_ADDR(sgx_enclave_id_t eid, void** retval)
{
	sgx_status_t status;
	ms_get_BN_usub_ADDR_t ms;
	status = sgx_ecall(eid, 6, &ocall_table_TestEnclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t get_ECDSA_free_ADDR(sgx_enclave_id_t eid, void** retval)
{
	sgx_status_t status;
	ms_get_ECDSA_free_ADDR_t ms;
