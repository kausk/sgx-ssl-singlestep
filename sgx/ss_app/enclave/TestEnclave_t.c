...
typedef struct ms_get_ECDSA_sign_ADDR_t {
	void* ms_retval;
} ms_get_ECDSA_sign_ADDR_t;

typedef struct ms_get_BN_mod_aq_ADDR_t {
	void* ms_retval;
} ms_get_BN_mod_aq_ADDR_t;

typedef struct ms_get_BN_usub_ADDR_t {
	void* ms_retval;
} ms_get_BN_usub_ADDR_t;

typedef struct ms_get_ECDSA_free_ADDR_t {
	void* ms_retval;
} ms_get_ECDSA_free_ADDR_t;
...
static sgx_status_t SGX_CDECL sgx_sign_single(void* pms)
{
	...
	ms->ms_retval = sign_single(_tmp_message, ms->ms_len);
...
static sgx_status_t SGX_CDECL sgx_get_ECDSA_sign_ADDR(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_get_ECDSA_sign_ADDR_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_get_ECDSA_sign_ADDR_t* ms = SGX_CAST(ms_get_ECDSA_sign_ADDR_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = get_ECDSA_sign_ADDR();


	return status;
}

static sgx_status_t SGX_CDECL sgx_get_BN_mod_aq_ADDR(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_get_BN_mod_aq_ADDR_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_get_BN_mod_aq_ADDR_t* ms = SGX_CAST(ms_get_BN_mod_aq_ADDR_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = get_BN_mod_aq_ADDR();


	return status;
}

static sgx_status_t SGX_CDECL sgx_get_BN_usub_ADDR(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_get_BN_usub_ADDR_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_get_BN_usub_ADDR_t* ms = SGX_CAST(ms_get_BN_usub_ADDR_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = get_BN_usub_ADDR();


	return status;
}

static sgx_status_t SGX_CDECL sgx_get_ECDSA_free_ADDR(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_get_ECDSA_free_ADDR_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_get_ECDSA_free_ADDR_t* ms = SGX_CAST(ms_get_ECDSA_free_ADDR_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = get_ECDSA_free_ADDR();
...
SGX_EXTERNC const struct {
	...
	8,
	{
		...
		{(void*)(uintptr_t)sgx_get_ECDSA_sign_ADDR, 0},
		{(void*)(uintptr_t)sgx_get_BN_mod_aq_ADDR, 0},
		{(void*)(uintptr_t)sgx_get_BN_usub_ADDR, 0},
		{(void*)(uintptr_t)sgx_get_ECDSA_free_ADDR, 0},
	}
};
